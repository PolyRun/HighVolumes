...
int main(int argc, char *argv[]){
    ...
    int walk_lower = 1, walk_upper = 10, walk_step = 1;
    ...
    cliFun.add(new CLIF_OptionNumber<int>(&walk_step, 'w', "c", "1", 1, 1000000));


    cliFun.claimOpt('s', "step_size range");
    // lower and upper bounds and step size of step_size parameter for volume algo
    // -> we try all configurations in range(lower, upper+1, step)
    int step_lower = 1600, step_upper = 10000, step_step = 1000;
    ...
    cliFun.add(new CLIF_OptionNumber<int>(&step_step, 's', "c", "1000", 1, 1000000));
